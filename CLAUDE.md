...
- **Scripts:** `npm run dev` (unchanged, Next dev server) · `npm run build` (**`next build && opennextjs-cloudflare build --skipNextBuild`** — compiles Next, then bundles the Worker into `.open-next/`. Cloudflare's CI runs `npm run build` by default, so this must be the full pipeline — do not change it back to plain `next build`. The `--skipNextBuild` flag matters: without it OpenNext calls `npm run build` itself and recurses forever) · `npm run build:next` (plain `next build`, for a quick compile check) · `npm run preview` (build + run in local `workerd`, port 8787 — **the real test**) · `npm run deploy` (build + `wrangler deploy`, needs a Cloudflare login) · `npm run cf-typegen` (types for bindings).
- **Cloudflare CI settings that work:** build command `npm run build`, deploy command `npx wrangler deploy` (wrangler detects the OpenNext project and delegates to `opennextjs-cloudflare deploy`). These are the dashboard defaults.
